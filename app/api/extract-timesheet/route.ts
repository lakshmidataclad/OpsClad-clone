...
async function createExtractionProgress(
  ...
  try {
    const { error } = await supabase.from("extraction_progress").insert({
      user_id: userId,
  ...
  try {
    ...
    if (checkError) {
      ...
      return NextResponse.json(
        {
          success: false,
          message: "An extraction is already in progress. Please wait.",
        },
        { status: 400 }
      );
    }
    ...
    if (gmailError || !gmailSettings) {
      return NextResponse.json(
        {
          success: false,
          message: "Gmail credentials not found. Please connect Gmail first.",
        },
        { status: 400 }
      );
    }
    ...
    if (employeeError || !employees || employees.length === 0) {
      return NextResponse.json(
        {
          success: false,
          message: "Employee data not found. Please upload employee CSV first.",
        },
        { status: 400 }
      );
    }
    ...
    if (projectError || !projects || projects.length === 0) {
      return NextResponse.json(
        {
          success: false,
          message: "Project data not found. Please upload project CSV first.",
        },
        { status: 400 }
      );
    }
    ...
    const progressCreated = await createExtractionProgress(
      userId,
      extractionId,
      searchMethod,
...
async function processTimesheetExtraction(
  ...
  try {
    ...
    projects.forEach((proj) => {
      ...
      const normalizedClient = proj.client
        .toLowerCase()
      ...
      if (employeeProjectMap[emailKey]) {
        ...
        console.warn(
          `Employee ${proj.employee_email} found in projects but not in employees table`
        );
        employeeProjectMap[emailKey] = {
    ...
    const pythonResults = await executePythonScript(
      extractionId,
      pythonInput,
      resultsFilePath
    );
    ...
    // --- HOLIDAY OVERRIDE LOGIC BEFORE INSERTING ---
    ...
    for (const entry of processedData) {
      ...
      const { data: holiday } = await supabase
        .from("holidays")
        .select("holiday_date")
        .eq("holiday_date", entry.date)
        .maybeSingle();

      const isHoliday = !!holiday;

      // 2. Override activity + hours when holiday
      let activity = entry.activity;
      let hours = entry.hours;

      if (isHoliday) {
        activity = "HOLIDAY";
        hours = entry.required_hours ?? entry.hours;
      }
    ...
    // This will also override existing WORK entries on holidays with HOLIDAY.
    ...
    console.log(
      `Successfully processed ${insertedCount} timesheet entries (includes updates to existing entries)`
    );
        ...
        try {
          ...
          if (results.success || results.extracted_data) {
            ...
            resolve({
              success: false,
              message:
                results.message || `Python process failed with code ${code}`,
              errors: [stderrBuffer],
            });
          }
        } catch (fileError: any) {
          ...
          resolve({
            success: false,
            message:
              "Python process completed but results file couldn't be read. " +
              `Exit code: ${code}`,
    ...
    const response = {
      success:
        ...
        (progressData.is_processing ||
          progressData.new_extracted_entries?.length > 0),
      ...
      status: {
        result: {
          ...
          total_entries_inserted_into_db:
            progressData.total_entries_inserted_into_db || 0,
